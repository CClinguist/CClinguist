# #更新：用于测275ms 300Kbps的profile下的dtw距离
#  0913： 用于测试ns3仿真得到的：comparative_experiment/ccanalyzer/data_ccanalyzer_stand  1:3
import torch
...
class queueOccuDataset_build(nn.Module): 
    ...
    def get_merged_df(self, host_path, capture_path):
        host_df = pd.read_csv(host_path, sep='\s+', names=['Time', 'Length', 'DataLength', 'SeqNum'], index_col=False)
        
        #print(host_df.tail())
        if pd.isna(host_df.iloc[-1]['SeqNum']):
            host_df = host_df.iloc[:-1]
        ...
        capture_df = pd.read_csv(capture_path, sep='\s+', names=['Type', 'Time', 'Length', 'DataLength', 'SeqNum'], index_col=False)
        #capture_df = pd.read_csv(capture_path, delimiter=',', names=['Type', 'Time', 'Length', 'DataLength', 'SeqNum'], index_col=False,nrows=4000)
        ...
        for _, host_row in host_df.iterrows():
            ...
            if not matching_seq_rows.empty:
                ...
                # if len(matching_seq_rows) > 1:
                #     print(f'maching_seq_rows: {matching_seq_rows}')
                #     print(f'host_row: {host_row}, closest_seq_row: {closest_seq_row}')
    ...
    def forward(self, path, train_data_num, test_data_num, target_env):
        ...
        alo_dic = {'htcp': 0, 'bbr': 1, 'vegas': 2, 'westwood': 3, 'scalable': 4, 'highspeed': 5, 'veno': 6, 'reno': 7, 'yeah': 8, 'illinois': 9, 'bic': 10, 'cubic': 11, 'pccLatency': 12, 'pccLoss': 13, 'astraea': 14}
        
        #alo_dic_ns3
        #alo_dic = {'htcp': 0, 'bbr': 1, 'vegas': 2, 'westwoodplus': 3, 'scalable': 4, 'highspeed': 5, 'hybla': 6, 'newreno': 7, 'yeah': 8, 'illinois': 9, 'bic': 10, 'cubic': 11}
        ...
        for env in envs:
            ...
            for alo in tqdm(alos):
                if alo.startswith("host"):
                    ...
                    elif alo_idx >= train_data_num and alo_idx < train_data_num+test_data_num:  # 测试数量
                    #else:
                        #print('get in test')
        ...
        #print(f'test_data.size(): {len(test_data)}, test_loader.size(): {len(test_loader)}')
...
def predict_with_voting(data_trace, test_labels, train_traces, train_labels, method, output_file):
    predictions = []
    ...
    if method[1] == 0:
        for label, traces in label_groups.items():
            ...
            with open(output_file, 'a+') as f:
                f.write(f'label: {label}, group_predictions: {group_predictions}, most_common: {most_common}\n')
...
def test_dtw_classify(data_path, batch_size, train_data_num, test_data_num, output_path, target_env, method):
    ...
    for i, input in tqdm(enumerate(test_loader)):
        ...
        output_file = f'{output_path}/result_classify_{target_env}.txt'
        predictions = predict_with_voting(data_trace, label, train_traces, train_labels, method, output_file)
    ...
    with open(f'{output_path}/result_classify.txt', 'a+') as f:
...
def classify_for_env(train_path, test_data_num, train_data_num, output_path, target_env, method, batch_size):
    print(target_env)   
    method_dic = {0: 'with padding', 1: 'without padding', 2: 'The category of the nearest point is the predicted label.', 3: 'The label group with the shortest average distance is the predicted label.'}
    with open(f'{output_path}/result_classify_{target_env}.txt', 'a+') as f:
        f.write(f'\n========== method: {method_dic[method[0]]} & {method_dic[method[1] + 2]} & {target_env} ==========\n')
    test_dtw_classify(train_path, batch_size, train_data_num, test_data_num, output_path, target_env, method)   # batch_size = test_data_num * cc_num
...
if __name__ == "__main__":
    ...
    output_path = './output/CCAnalyzer/test'
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for method in method_list:
            ...
            for target_env in envs:
                if target_env != "rtt_240ms_bdw_600Kbps":
                    continue
                # if(int(re.search(r'_(\d+)Kbps$', target_env).group(1)) > 1000):   
                #     batch_size = test_data_num * 12
                # else:
                #     batch_size = test_data_num * 15

                batch_size = test_data_num * 12
                futures.append(executor.submit(classify_for_env, train_path, test_data_num, train_data_num, output_path, target_env, method, batch_size))
    ...
    input_file = f'{output_path}/result_classify.txt'
    average_accuracy = calculate_average_accuracy(input_file)

    if average_accuracy is not None:
        print(f"average acc: {average_accuracy:.2f}%")
        with open(f'{output_path}/result_classify.txt', 'a+') as f:
            f.write(f'\naverage acc: {average_accuracy:.2f}%\n')
        
    else:
        print("cannot find valid data")
        with open(f'{output_path}/result_classify.txt', 'a+') as f:
            f.write(f'\ncannot find valid data\n')
