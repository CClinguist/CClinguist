...
data_path = 'data_12ccs/simulation_pk2hk_12_20'
...
result_dir = 'results_12CCs_coef/log_output/test_log'
output_dir = 'results_12CCs_coef/analyze'


all_files = sorted([f for f in os.listdir(result_dir) if f.endswith('.txt')])
...
for batch_index, file_names in enumerate(batches):

    for file_name in file_names:
        dic_alo = {}
        dic_env = {}
            
        with open(f'{result_dir}/{file_name}', 'r') as f:
            lines = f.readlines()
        ...
        f_log=open(f'{output_dir}/{file_name}_analyze.txt','w')
        ...
        print("file ",file_name,"has been analyzed.")
        f_log.close()
    ...
    USE_ACCURACY_IF_FAIL = False  


    final_output_dir = "results_12CCs_coef/analyze_final"
    analyze_dir = 'results_12CCs_coef/analyze'
    ...
    for file in input_files:
        ...
        with open(f'{analyze_dir}/{file}_analyze.txt', 'r') as f:
            content = f.read()

        matches = re.findall(
            r"Name: (\w+), Correct Recognitions: (\d+), Total Tests: (\d+), Accuracy: ([0-9.]+), Voting Result: (\w+)",
            content
        )

        for name, correct, total, acc, result in matches:
            score = float(acc) if (result == "Fail" and USE_ACCURACY_IF_FAIL) else (1.0 if result == "Pass" else 0.0)
            algo_scores[name].append(score)

    final_accuracies = {}
    for algo, scores in algo_scores.items():
        final_accuracies[algo] = sum(scores) / len(scores)


    Total_Final_Accuracy = sum(final_accuracies.values()) / len(final_accuracies)


    output_file_name = os.path.splitext(os.path.basename(input_files[0]))[0] + "_analyzed.txt"
    output_path = os.path.join(final_output_dir, output_file_name)

    with open(output_path, 'w') as out:
        out.write("Algorithm Final Accuracy Results (Pass=1, Fail=0 or Accuracy if configured):\n")
        for algo, acc in sorted(final_accuracies.items()):
            out.write(f"{algo:12s}: {acc:.4f} ({len(algo_scores[algo])} files)\n")
        out.write("\nTotal Final Accuracy (mean over all algorithms): {:.4f}\n".format(Total_Final_Accuracy))
