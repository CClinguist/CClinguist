...
def build_model(config=None):
    ...
    # with wandb.init(config=config):
        ...
        min_test_accuracy = 0
        if os.path.exists(f'dis_log_simccas/{config.num_cluster}') == False:
            os.makedirs(f'dis_log_simccas/{config.num_cluster}')
        
        f=open(f'dis_log_simccas/{config.num_cluster}/{config.unknown_cca}'+'.txt', 'a+')
        
    
        
        if config.variant_same:
...
def parse_arguments():
    ...
    parser.add_argument('--test_path', type=str, default='data/simulation_new_CCs_test', help='Test data path')
    parser.add_argument('--constrast_path', type=str, default='data/simulation_15CCs_train', help='Contrast data path')
    ...
    parser.add_argument('--unknown_cca', type=str, default='pccLatency2', help='Unknown CCA type')
    parser.add_argument('--variant_same', type=bool, default='True', help='SimCCAs same flag')
...
if __name__ == "__main__":
    ...
    if online_flag:
        ...
        option_variant_same = [True, False]  # True: pccLatency, pccLoss regarded as pcc; False: pccLatency, pccLoss regarded as different ccas
        
        unknown_ccas = ['pccvariant1','pccvariant2']
        
        for i in option_variant_same:
            config.variant_same = option_variant_same[i]
            if config.variant_same:
                num_cluster = 13
            else:
                num_cluster = 15
            dic_cluster_model = {
                13: 'checkpoint/checkpoint_pcc.pth.tar',
                15: 'checkpoint/checkpoint_15ccas.pth.tar'
            }
            dic_culster_test_path = {
                13: 'data/simlarccas_test',
                15: 'data/simlarccas_test',
            }
            dic_cluster_profile = {  #the best profile configed by profile-tree       
                13: 'rtt_200ms_bdw_600Kbps',   #13 known ccas: 12 + pcc (pccLatency,pccloss regarded as pcc)
                15: 'rtt_400ms_bdw_400Kbps'    #simccas, 12 knownccas + pccLatency + pccloss
            }

            for unknown_cca in unknown_ccas:
                print(f'Running for unknown_cca: {unknown_cca}, variant_same: {config.variant_same}')
                ...
                config.constrast_path = 'data/simulation_15CCs_train'
