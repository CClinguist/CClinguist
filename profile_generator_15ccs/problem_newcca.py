...
class Dataset(object):
    ...
    def make_dataset_1209(self, batch_size, train=True, train_data_num=1, num_cluster = 13, only_newcca=False):
        ...
        if train == True or only_newcca == False:
            if num_cluster == 15:
                ...
                alo_dic={'htcp': 0, 'bbr': 1, 'vegas': 2, 'westwood': 3, 'scalable': 4, 'highspeed': 5, 'veno': 6, 
                        'reno': 7, 'yeah': 8, 'illinois': 9, 'bic': 10, 'cubic': 11}
            elif num_cluster == 13:
                alo_dic={'htcp': 0, 'bbr': 1, 'vegas': 2, 'westwood': 3, 'scalable': 4, 'highspeed': 5, 'veno': 6, 
                    'reno': 7, 'yeah': 8, 'illinois': 9, 'bic': 10, 'cubic': 11, 'pccLatency': 12}
            else:
                alo_dic={'htcp': 0, 'bbr': 1, 'vegas': 2, 'westwood': 3, 'scalable': 4, 'highspeed': 5, 'veno': 6, 
                    'reno': 7, 'yeah': 8, 'illinois': 9, 'bic': 10, 'cubic': 11, 'pccLatency': 12,  'astraea': 13}
        else:
            if only_newcca == True:
                if num_cluster == 12:    
                    alo_dic = {'pccLatency': 12, 'astraea': 13, 'pccLoss': 14}
                if num_cluster == 13:    
                    alo_dic = {'astraea': 13, 'pccLoss': 14}
                if num_cluster == 14:   
                    alo_dic = {'pccLoss': 14}
        ...
        for env in envs:   
            ...
            if '1000Kbps' in env:
                continue
            if 'rtt_80ms_bdw_600Kbps' in env:  
                continue
            if 'rtt_80ms_bdw_400Kbps' in env:
                continue
            env_id+=1
            ...
            for alo_name, data_list in x_trace_tmp.items(): 
                ...
                if len(sorted_indices) == 1: 
                    ...
                    if train:   
                        ...
                        if len(indices_to_add) != train_data_num:
                            print(f"Env {env} has less than 9 data, skipping.")
        ...
        a = data_trace.reshape([len(self.env_ids),self.num_alos,self.input_data_num,data_trace.shape[-1]]) 
    ...
    def load_action_dtw(self, num_cluster=13):

        dtw_result=None

        if num_cluster == 12:
            target_alo_list = ['htcp', 'bbr', 'vegas', 'westwood', 'scalable', 'highspeed', 'veno', 
                        'reno', 'yeah', 'illinois', 'bic', 'cubic']
        elif num_cluster == 13:
            target_alo_list = ['htcp', 'bbr', 'vegas', 'westwood', 'scalable', 'highspeed', 'veno', 
                        'reno', 'yeah', 'illinois', 'bic', 'cubic', 'pccLoss']
        elif num_cluster == 14:
            target_alo_list = ['htcp', 'bbr', 'vegas', 'westwood', 'scalable', 'highspeed', 'veno', 
                        'reno', 'yeah', 'illinois', 'bic', 'cubic', 'pccLoss', 'pccLatency']
        else:
            target_alo_list = ['htcp', 'bbr', 'vegas', 'westwood', 'scalable', 'highspeed', 'veno', 
                        'reno', 'yeah', 'illinois', 'bic', 'cubic', 'pccLoss', 'pccLatency', 'astraea']

        
        
        if self.dtw_path is None:
            
            dtw_result=compute_dtw([self.x_trace,self.y,self.x_env_id], num_cluster, data_path=None)

            if dtw_result is not None:
                output_file = "/home/ml4net/LJH/EFAAA/1209_cclinguist/dtw_matrix_14CCs.txt"
                print(f"[Debug] dtw_path is None (training mode). Save computed dtw to {output_file}")
                save_dtw_as_readable_txt(dtw_result, target_alo_list, output_file)
                print("[Debug] dtw_result has been saved in tabular format.")
            else:
                print("[Warning] dtw_result is None, skip saving.")
