...
def get_options(args=None):
    ...
    if flag == 0 : 
        ...
        parser.add_argument('--train_data_path', type=str, default=os.path.join(base_dir, 'data_15ccs', 'simulation_pk2hk_15CCs'), 
                            help='Path to training data')
        parser.add_argument('--test_data_path', type=str,default=os.path.join(base_dir, 'data_15ccs', 'simulation_15CCs'), 
        ...
        parser.add_argument('--load_path', type=str,  default = os.path.join(base_dir, 'model_rl_paras', '0606-1837-38', 'epoch-4.pt'))
        parser.add_argument('--load_path_classifier', type=str, 
                            default=os.path.join(base_dir,'models','checkpoint_9000.pth.tar'),
                            help='Path to load classifier model from')          
        parser.add_argument('--num_cluster', type=int, default=15, help='Number of clusters')    
        parser.add_argument('--dtw_path', type=str, default=os.path.join(base_dir,'15_dtw_pcc_latency.txt'), help='Path to DTW data')       
    ...
    parser.add_argument('--lr_critic', type=float, default=0.005, help='Learning rate for critic,last=0.002,0,02')
    ...
    parser.add_argument('--removed_prob', type=float, default=0.2, help='The probability of removed alos,need to be divided by num_cluster')
    ...
    #reward
    parser.add_argument('--dtw_weight', type=float, default=0, help='reward of dtw')
    parser.add_argument('--base_penalty', type=float, default=1, help='penalty of can not search reward')
    ...
    parser.add_argument('--direction_penalty', type=float, default=1, help='penalty of first direction')  
    ...
    parser.add_argument('--coef_dtw', type=float, default=0, help='Coefficient for dtw reward')    
    parser.add_argument('--coef_base_penalty', type=float, default=1, help='Coefficient for base penalty at max depth')  
    #w1,w2,w3
    parser.add_argument('--coef_acc_reward', type=float, default=1, help='Coefficient for correct classification reward')  
    parser.add_argument('--coef_obvious_reward', type=float, default=1,help='Coefficient for obvious confidence reward')   
    parser.add_argument('--coef_search_penalty', type=float, default=1, help='Coefficient for search step penalty')       
    
    parser.add_argument('--coef_direction_penalty', type=float, default=1, help='Coefficient for wrong direction penalty') 
    ...
    log_output_dir = os.path.join(base_dir, 'results_15CCs_coef', 'log_output')
    train_log_dir = os.path.join(log_output_dir, 'train_log')
    test_log_dir = os.path.join(log_output_dir, 'test_log')
    model_dir = os.path.join(base_dir, 'model_rl_paras')
    
    log_steps_dir = os.path.join(base_dir, 'results_15CCs_coef', 'log_steps')
    log_steps_train_dir = os.path.join(log_steps_dir, 'train_log')
    log_steps_test_dir = os.path.join(log_steps_dir, 'test_log')
    result_dir = os.path.join(base_dir, 'results_15CCs_coef', 'analyze')


    for d in [train_log_dir, test_log_dir, model_dir,
              log_steps_train_dir, log_steps_test_dir,
              result_dir]:
